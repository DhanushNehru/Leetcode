 ...
 * @return {number}
 */
// Function to find the minimum number of buses required to reach the target stop from the source stop
var numBusesToDestination = function(routes, source, target) {
  ...
  // This array will contain flags indicating whether a route has been visited or not
  let visitedRoutes = new Array(routes.length);
  
  // While there are more stops to visit
  while (!stopsToVisit.isEmpty()) {
    // Get the queued stop and the number of buses it took to get there
    ...
    for (let route of stopRoutes.get(stop)) {
      ...
      for (let nextStop of routes[route]) {
        ...
        if (!visitedStops.has(nextStop)) {
          ...
          stopsToVisit.enqueue([nextStop, bus + 1]);
        }
      }
